...
def process_sequence(seq_id, id_y, train_save_path):
    ...
    data = Data(x=cds_feature, edge_index=cds_edge_index, y=y)
...
def new_dataset(label_path, save_path):
    ...
    for line in label_f:
        id_y[line.split('\t')[0]] = int(line.split('\t')[2].strip())
        sequences.append(line.split('\t')[0])
    label_f.close()

    with multiprocessing.Pool(processes=16) as pool:
        pool.starmap(process_sequence, [(seq_id, id_y, save_path) for seq_id in sequences])
